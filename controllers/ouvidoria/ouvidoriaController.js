...
const sendEmailManifestacaoController = require('./sendEmailManifestacaoController');
...
module.exports = {
    ...
    newManifestacao(req, res) {
        try {
            ...
            const arquivo = req.files?.[0]?.filename ? `${process.env.BASE_URL}/uploads/manifestacao/${req.files[0]?.filename}` : '';
    ...
    updateManifestacao(req, res) {
        ...
        const arquivo = req.files[0]?.filename ? `${process.env.BASE_URL}/uploads/manifestacao/${req.files[0]?.filename}` : dataForm.arquivo;
